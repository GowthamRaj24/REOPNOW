...
const Call = () => {
  ...
  const [messageClient,setClientMessage]=useState('');
  ...
  useEffect(() => {
    const socket = io("http://172.22.192.144:5000");
    setSocket(socket);
    if(socket===null){
      setEndSession(true);
      return;
  ...
  const handleSendMessage=()=>{
    ...
    const obj = {};
    obj.room = roomId;
    obj.message = messageClient;
    obj.name=myName;
    socket.emit("send-message", JSON.stringify(obj));
    SetMessages([...messages,{id:'1',name:'Me',message:messageClient}]);
    setClientMessage('');
  }
    ...
    <div className="callContainer">
      ...
      <div className="rightcallContainer">
        ...
        <div className="sendMessage">
          <input className="enterText" value={messageClient} type="text" placeholder="Type something" onChange={handleInputChange}/>
